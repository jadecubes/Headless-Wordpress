...
render() {
  ...
  dst="$2"
  envsubst \
    '${ADMIN_HOST} ${API_HOST} ${ADMIN_ORIGIN} ${FRONTEND_ORIGIN} ${UPSTREAM_HOST} ${UPSTREAM_PORT} ${SSL_CERT_PATH} ${SSL_KEY_PATH} ${PUBLIC_TLS_PORT} ${FRONTEND_ORIGIN}' \
    < "$src" > "$dst"
...
render "$TEMPLATE_ROOT/servers/site-frontend.conf.tpl"     "$RENDER_ROOT/servers/site-frontend.conf"
